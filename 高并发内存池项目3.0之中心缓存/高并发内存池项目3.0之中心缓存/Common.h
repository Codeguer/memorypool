...
struct Span
{
	...
	size_t _objsize;		// 切出来的单个对象的大小：即span管理的这块空间，切给某个对象，那么这块空间
	//就只按该对象的大小进行切割，当有不同大小的对象时就生成新的span；为什么要这样做呢：
	//这样就可以根据不同大小的对象合理的分配span合适的大小，不会造成给小对象分配过多的页，
...
// thread cache一次从中心缓存获取多少个
	//小对象的话central cache就多给几个
	//大对象的话central cache就少给几个
static size_t NumMoveSize(size_t size){//size表示单个对象的大小
	if (size == 0)
		return 0;

	// [2, 512]，thread cache一次从central cache批量移动多少个对象的(慢启动)上限值
